...
pmeta_tidy <- wfs |>
  rowwise() |>
  mutate(tidy_meta = list(func_eval(.data$func)(pmeta, status))) |>
  select("name", "tidy_meta") |>
  tibble::deframe()
get_libids <- function(tbl) {
  tbl |>
    select(contains("LibraryID")) |>
    unlist() |>
    unique()
}
libids <- pmeta_tidy |>
  purrr::map(get_libids) |>
  unlist() |>
  unique() |>
  sort()
```
...
lims <- lims_raw |>
  select(
    "timestamp", "subject_id", "sample_id", "sample_name", "library_id",
    "external_subject_id", "external_sample_id",
    "project_owner", "project_name", "type", "assay", "phenotype", "source", "quality", "topup", "workflow"
  )
```
...
clrs1 <- get_colours(length(unique_sbj)) |>
  ...
  mutate(sbj_url = get_sbj_url(.data$sbjid, colour = .data$color, account = "pro"))
...
```{r show_sumy_tbls}
status_count(pmeta_status_count)
sbj_sumy <- pmeta_sumy |>
...
pmeta |>
  ...
  mutate(
    SubjectID = if_else(is.na(.data$sbj_url), get_sbj_url(.data$SubjectID), .data$sbj_url),
...
pmeta |>
  ...
  lyr::left_join(lims, by = c("LibraryID", "SampleID", "SubjectID")) |>
  lyr::left_join(clrs1 |> select(sbjid, sbj_url), by = c("SubjectID" = "sbjid")) |>
  mutate(
    SubjectID = if_else(is.na(.data$sbj_url), get_sbj_url(.data$SubjectID), .data$sbj_url),
    durationMin = round(as.numeric(difftime(end, start, units = "mins")))
  ) |>
