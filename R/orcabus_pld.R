...
#' Payload Tidy dragen-tso500-ctdna - *Note there is an issue with "IndividualId" returned, does not match lims
#'
#' @param pld List with dragen-tso500-ctdna (previously cttsov2) workflow parameters.
#'
#' @return A tidy tibble.
#' @export
pld_dragentso500ctdna <- function(pld) {
  payload_okay(pld)
  pdata <- pld[["data"]]
  id <- pld[["orcabusId"]]
  # collapse FastqRgidList into single string
